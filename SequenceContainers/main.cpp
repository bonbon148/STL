...
template<typename T>
void vector_propeties(const std::vector<T>& vec)
{
...
void main()
{
	...
	vector_propeties(i_vec);
	i_vec.push_back(55);
	i_vec.push_back(89);
	i_vec.shrink_to_fit();  //Приравнивает вместительность вектора до его ффактического размера
	i_vec.reserve(25); //Резервирует доп. память
	vector_propeties(i_vec);
	//i_vec.resize(15);
	...
	i_vec.data(); //Возвращает RAW-массив, принадлежащий Вектору.
	// IA - 32
	//std::vector<double> d_vec;
	//vector_propeties(d_vec);
...
#ifdef STL_DEQUE
	//deque - это контейнер, который хранит данные в виде набора динамических массивов.
//deque может объединять массивы в массив, или в список, в зависимости от реализации.
//deque - Duble-Ended-Queue
	//На уровне пользователя deque отличается от vеctor тем, что может добавлять значения 
	//непостредственно в начало контейнера.
	...
	i_deque.push_front(34);
	i_deque.push_front(55);
	i_deque.push_front(89);
	...
	i_flist.remove_if([](const int value) { return value % 2 == 0; });
	for (int i : i_flist)
	{
		cout << i << tab;
	}
...
[]		- показывает начало lambda-выражения;
